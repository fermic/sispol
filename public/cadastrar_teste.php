...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
 ...
 $stmt = $pdo->prepare("
    ...
    (SituacaoID, OrigemID, TipoID, NumeroProcedimento, DataFato, DataInstauracao, MotivoAparente, EnderecoFato, 
    ...
    :EnderecoFato, :EscrivaoID, :DelegadoID, :DelegaciaID, NOW())
");
...
$stmt->execute([
    ...
    ':MotivoAparente' => $_POST['motivo_aparente'],
    ':EnderecoFato' => $_POST['endereco_fato'],
...
foreach ($_POST['investigados'] as $investigado) {
    ...
    if (!empty($investigado['nome']) || (isset($investigado['ignorado']) && $investigado['ignorado'] === '1')) {
        $nomeInvestigado = isset($investigado['ignorado']) && $investigado['ignorado'] === '1' ? 'Ignorado' : $investigado['nome'];
        $stmt = $pdo->prepare("
...
function adicionarCrime(vitimaIndex) {
    ...
    crimeDiv.innerHTML = `
        <div class="col-md-6">
            <select name="vitimas[${vitimaIndex}][crimes][${crimeIndex}][id]" class="form-control">
                <option value="">Selecione um Crime</option>
                <?php foreach (buscar_opcoes($pdo, 'Crimes', 'ID', 'Nome') as $crime): ?>
                    <option value="<?= $crime['id'] ?>"><?= $crime['nome'] ?></option>
                <?php endforeach; ?>
...
document.addEventListener('DOMContentLoaded', function () {
 ...
 document.getElementById('tipo_id').addEventListener('change', function () {
    ...
    let categoria;
    if (tipoId === '1') { // Substitua "1" com o ID correspondente para "IP"
        categoria = 'IP';
    } else if (tipoId === '2') { // Substitua "2" com o ID correspondente para "VPI"
        categoria = 'VPI';
    } else {
